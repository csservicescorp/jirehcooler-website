...
const srcPath = join(root, 'public', 'images', 'jireh-originals', 'new-jireh-images', 'office.png');
const outDir = join(root, 'public', 'images', 'homepage');
const manifestPath = join(root, 'src', 'data', 'image-manifest.json');

mkdirSync(outDir, { recursive: true });

const basename = 'jireh-cooler-office-hero';
...
for (const width of widths) {
  ...
  const buf = await resized.clone().jpeg({ quality: 85, mozjpeg: true }).toBuffer();
  const info = await sharp(buf).metadata();
  writeFileSync(jpgOut, buf);
  await resized.clone().webp({ quality: 82 }).toFile(webpOut);
  await resized.clone().avif({ quality: 60 }).toFile(avifOut);
...
const publicPath = `/images/homepage/${basename}.jpg`;
const [full, sm] = sizes;
manifest[publicPath] = {
  width: full.width,
  height: full.height,
